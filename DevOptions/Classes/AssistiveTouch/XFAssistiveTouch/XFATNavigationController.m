...
- (void)loadView {
    ...
    UIImage *image = nil;
    if (_delegate && [_delegate respondsToSelector:@selector(imageForAssistiveTouch)]) {
        image = [_delegate imageForAssistiveTouch];
    }

    _contentItem = [XFATItemView assistiveItemWithImage:image];
...
- (void)spread {
    ...
    for (int i = 0; i < count; i++) {
        ...
        [UIView animateWithDuration:[XFATLayoutAttributes animationDuration] animations:^{
            item.frame = [XFATPosition positionWithIndex:i].frame;
            item.alpha = 1;
        }];
    }
...
- (void)shrink {
    ...
    [UIView animateWithDuration:[XFATLayoutAttributes animationDuration] animations:^{
        self.viewControllers.lastObject.backItem.center = self.contentPoint;
        self.viewControllers.lastObject.backItem.alpha = 0;
    }];
    
    [UIView animateWithDuration:[XFATLayoutAttributes animationDuration] animations:^{
        self.contentView.frame = CGRectMake(0, 0, [XFATLayoutAttributes itemImageWidth], [XFATLayoutAttributes itemImageWidth]);
        ...
        self.viewControllers = [NSMutableArray arrayWithObject:self.viewControllers.firstObject];
...
- (UIStatusBarStyle)preferredStatusBarStyle {
    if (_delegate && [_delegate respondsToSelector:@selector(preferredStatusBarStyle)]) {
